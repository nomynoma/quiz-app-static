...
2. ダウンロードボタンで JPEG 画像として保存が可能か確認
...
2. 次のレベルがアンロックされるか確認
3. メダルバッジをクリックして合格証モーダルが表示されるか確認
4. モーダルから合格証を再ダウンロードできるか確認

### 5. 超級モード

1. 各ジャンルの上級をクリア
2. ジャンル選択画面で「超級」ボタンが表示されるか確認
3. 超級に挑戦して全問正解すると合格証が発行される

### 6. エクストラステージ

1. 全6ジャンルの上級をクリア
2. ジャンル選択画面に「エクストラステージに挑戦」ボタンが表示されるか確認
3. エクストラステージに挑戦
4. 全問正解で「殿堂入り」として記録される
5. 「TOP10ランキングを見る」ボタンからランキング画面にアクセス可能か確認
...
- html2canvasライブラリが読み込まれていない → `pass.html` で html2canvas CDN が正しく読み込まれているか確認
- ブラウザコンソールにエラーが出ていないか確認

### 合格証モーダルで画像が表示されない

**症状**: ジャンル選択画面でメダルバッジをクリックしてもモーダルに画像が表示されない

**原因と対策**:
- html2canvasライブラリが読み込まれていない → `genre-select.html` で html2canvas CDN が正しく読み込まれているか確認
- `genre-select.css` が読み込まれていない → `genre-select.html` で CSS が正しく読み込まれているか確認
- 背景画像の相対パスが間違っている → `genre-select.js` で `../../imgs/` を `imgs/` に変換しているか確認
- ブラウザコンソールでCORSエラーやその他のエラーを確認

### エクストラステージが表示されない

**症状**: 全ジャンルの上級をクリアしてもエクストラステージボタンが表示されない

**原因と対策**:
- 全6ジャンルの上級（level="上級"）をクリアしているか確認
- ブラウザのlocalStorageに合格証メタデータ（`cert_1-3` ～ `cert_6-3`）が存在するか確認
- テスト用に `unlockExtraStage()` 関数をブラウザコンソールで実行（`config.js` に定義）
- ページをリロード

### 超級モードが解放されない

**症状**: 上級をクリアしても超級ボタンが表示されない

**原因と対策**:
- 各ジャンルの上級（level="上級"）をクリアしているか確認
- 合格証メタデータ（例: `cert_1-3`）がlocalStorageに保存されているか確認
- ページをリロード
...
1. CSSファイルを編集:
   - 全体のデザイン: `public/css/style.css`
   - ジャンル選択画面: `public/css/genre-select.css`
   - ジャンル別のクイズ画面: `public/genres/genreN/style.css`
2. Gitにコミット&プッシュ
3. GitHub Pagesの場合、数分で自動反映

### 合格証のスタイルを変更する場合

1. `public/css/genre-select.css` を編集
2. 合格証テキストのスタイル（`.certificate-title`, `.certificate-name` など）を変更
3. ジャンル別の色（`.certificate-genre1` ～ `.certificate-genre6`, `.certificate-extra`）を変更
4. Gitにコミット&プッシュ
